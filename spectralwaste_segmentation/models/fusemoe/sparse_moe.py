...
import numpy as np
from .activations import ACT2FN  # EDIT: use relative import within spectralwaste module
from .config import MoEConfig  # EDIT: use relative import within spectralwaste module
import pdb
...
class SparseDispatcher(object):
    ...
    def combine(self, expert_out, multiply_by_gates=True):
        ...
        stitched = torch.cat(expert_out, 0)  # EDIT: experts now return logits, no exp
        ...
        # EDIT: keep outputs in linear/logit space
        return combined  # EDIT: keep logits in linear space
...
class MLP(nn.Module):
    def __init__(self, config:MoEConfig, input_size:int, output_size:int, hidden_size:int):
        ...
        self.log_soft = nn.Identity()  # EDIT: keep logits for CrossEntropyLoss
...
class MoE(nn.Module):
    ...
    def _get_logits(self, x, train, noise_epsilon, idx=None):
        ...
        if self.noisy_gating:
            ...
            logits = clean_logits
            noisy_logits = clean_logits  # EDIT: define for non-noisy gating path
            noise_stddev = torch.zeros_like(clean_logits)  # EDIT: define for non-noisy gating path
